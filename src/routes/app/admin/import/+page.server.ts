...
export const actions: Actions = {
  migrate: async ({ request, locals, cookies }) => {
    ...
    const githubUrl = (data.get('githubUrl') as string)?.trim()
    ...
    try {
      const result = await apiPost<{ name: string }>(`/orgs/${orgId}/repos/migrate`, locals.session, {
        githubUrl,
        githubToken,
        teamId,
        private: true,
      ...
      return { success: true, repoName: result.name, teamName }
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Migration failed'
      if (message.includes('already exists')) {
        return fail(409, { error: 'Repository already exists in gittan' })
      }
