<script lang="ts">
  ...
  let fileContent = $derived((data.fileContent ?? null) as string | null)
  ...
  async function copyLineLink(): Promise<void> {
    if (!selectedLines) return
    const hash = selectedLines.start === selectedLines.end
      ? `#L${selectedLines.start}`
      : `#L${selectedLines.start}-L${selectedLines.end}`
    await navigator.clipboard.writeText(`${window.location.origin}${page.url.pathname}${hash}`)
    linkCopied = true
  ...
  {#if fileContent !== null}
    <div class="mb-2 flex justify-end">
      {#if selectedLines}
        ...
        >
          {linkCopied ? 'Link copied' : 'Copy link to lines'}
        </button>
      {/if}
    </div>
    <div class="bg-surface-900 border border-surface-800 rounded-lg overflow-hidden">
      <pre class="text-sm font-mono p-4 overflow-x-auto"><code>{#each fileContent.split('\n') as line, i}{@const lineNum = i + 1}{@const isSelected = selectedLines !== null && lineNum >= selectedLines.start && lineNum <= selectedLines.end}<a href="#L{lineNum}" class="inline-block w-10 text-right pr-4 select-none no-underline {isSelected ? 'text-accent-400' : 'text-surface-600 hover:text-surface-400'}">{lineNum}</a><span class="{isSelected ? 'bg-accent-400/10' : ''}">{line}</span>
{/each}</code></pre>
    </div>
  {:else}
    ...
    <div class="border border-surface-800 {latestCommit ? 'rounded-b-lg' : 'rounded-lg'}">
      ...
      {#if sortedEntries.length === 0}
        <div class="px-4 py-8 text-center text-sm text-surface-600">Empty directory</div>
      {/if}
    </div>
